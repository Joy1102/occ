...
depth_gt_path = './data/depth_gt'  #python ./tools/gen_data/gen_depth_gt.py
...
voxel_x = (point_cloud_range[3] - point_cloud_range[0]) / occ_size[0]  # 0.4  0.2？
voxel_y = (point_cloud_range[4] - point_cloud_range[1]) / occ_size[1]
voxel_z = (point_cloud_range[5] - point_cloud_range[2]) / occ_size[2]
voxel_channels = [64, 64*2, 64*4, 64*8]
empty_idx = 0  # noise 0-->255
num_cls = 17  # 0 free, 1-16 obj
visible_mask = False #不使用可见性掩码
img_norm_cfg = None #不使用归一化/使用默认归一化函数

cascade_ratio = 4 #级联缩小比例
...
data_config={
    'cams': ['CAM_FRONT_LEFT', 'CAM_FRONT', 'CAM_FRONT_RIGHT',
             'CAM_BACK_LEFT', 'CAM_BACK', 'CAM_BACK_RIGHT'],    #6张图
    'Ncams': 6,
    # 'input_size': (256, 704),
    'input_size': (896, 1600),
    'src_size': (900, 1600), #相机分辨率
    # image-view augmentation
    'resize': (-0.06, 0.11),
    'rot': (-5.4, 5.4),
    'flip': True,
    'crop_h': (0.0, 0.0),     #不进行垂直方向裁剪
    'resize_test': 0.00,      #不缩放
}
...
grid_config = {
    ...
    'dbound': [2.0, 58.0, 0.5], #深度；最小值最大值步长
}
numC_Trans = 80
...
model = dict(
    ...
    img_backbone=dict(
        ...
        type='ResNet',
        depth=50,     #指定ResNet深度（ResNet-50）
        num_stages=4,   #ResNet-50有四阶段
        out_indices=(0, 1, 2, 3),  #提取所有四个阶段的特征
        frozen_stages=0,  #不冻结阶段 可对所有层进行微调
        with_cp=True,  #使用CheckPoint
        norm_cfg=dict(type='SyncBN', requires_grad=True),  #同步批归一化 允许梯度反向传播
        norm_eval=False,
        style='pytorch'),
    img_neck=dict(
        type='SECONDFPN',
